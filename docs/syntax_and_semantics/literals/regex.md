...
- `m`: multiline matching (`PCRE_MULTILINE`): The _start of line_ (`^`) and _end of line_ (`$`) metacharacters match immediately following or immediately before internal newlines in the subject string, respectively, as well as at the very start and end.
