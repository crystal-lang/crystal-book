...
A connection is one of the key parts when working with databases. It represents the _runway_ through which statements travel from our application to the database.
...
In the next example we will let the `database` object _to manage the connections by itself_, like this:
