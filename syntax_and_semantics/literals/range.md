...
NOTE: Range literals are often wrapped in parenthesis, for example if it is meant to be used as the receiver of a call. `0..5.to_a` without parenthesis would be semantically equivalent to `0..(5.to_a)` because method calls and other operators have higher precedence than the range literal.
